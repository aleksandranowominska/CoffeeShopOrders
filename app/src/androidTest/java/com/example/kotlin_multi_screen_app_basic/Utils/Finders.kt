...
fun findViewById(@IdRes id: Int, index: Int = 0): ViewInteraction =
    onView(withIndex(withId(id), index))

fun waitUntilAllDisplayed(vararg interactions: ViewInteraction, timeout: Long = LONG_TIMEOUT) =
    waitUntil(
...
private fun waitUntil(condition: () -> Boolean, message: () -> String, timeout: Long) =
    runBlocking {
        try {
            withTimeout(timeout) {
                while (isActive) {
